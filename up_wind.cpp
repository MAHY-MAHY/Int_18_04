...
double Up_wind:: delta_zmx(int i){
  ...
  if(m_nFonc==0){
    ...
    if(((i)%m_M->getNbCellx())==0)
      {
	delta=m_M->getValueCell(i)-m_M->getValueCell(i+m_M->getNbCellx()-1);
      }
    else{
      delta =m_M->getValueCell(i)-m_M->getValueCell(i-1);
    }
...
double Up_wind :: delta_zmy(int i,int j){
  double delta;
  delta=m_M->getValxy(i,j)-m_M->getValxy(i,j-1);
  return delta ;   
}
double Up_wind :: delta_zpy(int i,int j){
  double delta;
  delta =m_M->getValxy(i,j+1)-m_M->getValxy(i,j);
  return delta ; 
}
...
double Up_wind :: sjy(int i,int j){
  double s;
  double delta_m, delta_p;
  delta_m=delta_zmy(i,j);
  delta_p=delta_zpy(i,j);
...
double Up_wind :: z_plusy(int i,int j){
  double z;
  z=m_M->getValxy(i,j)-0.5*m_M->getDyCell(i+m_M->getNbCellx())*sjy(i,j);
  return z;    
}

double Up_wind :: z_moinsy(int i,int j){
  double z;
  z=m_M->getValxy(i,j)+0.5*m_M->getDyCell(0)*sjy(i,j);
  return z;    
}
...
double Up_wind ::psiy(int i,double u){ ///// pour le cas 2 fonction pas terminé !!!!!
  ...
  y=i%m_M->getNbCellx();
  if(u>0){
    psi_p=u*z_moinsy(x,y);
    psi_m=u*z_moinsy(x,y-1);
  }
  else if(u<0){
    psi_p=u*z_plusy(x,y+1);
    psi_m=u*z_plusy(x,y-1);
  }
  psi=psi_p-psi_m;
  return psi;
} 
