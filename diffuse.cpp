...
double Diffuse:: phi(int i , int j,double grad_x,double grad_y){
  ...
  //phi=std::max(phi,tmp);;
  // phi=(phi_demipp(i,j,grad_x,grad_y)+phi_demimp(i,j,grad_x,grad_y)+phi_demimm(i,j,grad_x,grad_y)
  //     +phi_demipm(i,j,grad_x,grad_y))*0.25;
...
void Diffuse::saveMaillage(int compt)
{
  char fichier1[15]="fichier_";
  char fichier2[15]="fichier_";
  std::sprintf(&fichier1[8],"%d",compt);
  std::sprintf(&fichier2[8],"%d",compt+1);
  std::ofstream myfile;
  std::ofstream myfile2;
  std::ofstream myfile3;
  myfile.open(fichier1,std::ios::trunc);
  myfile2.open(fichier2,std::ios::trunc);
